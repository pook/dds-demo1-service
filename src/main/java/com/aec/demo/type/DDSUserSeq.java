...
/**
* A sequence of DDSUser instances.
*/
public final class DDSUserSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DDSUserSeq (int initialMaximum) {
        super(DDSUser.class, initialMaximum);
    }

    public DDSUserSeq (Collection<?> elements) {
        super(DDSUser.class, elements);
    }

    public  DDSUser get(int index) { 
        return (DDSUser) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DDSUser.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DDSUser objects (beyond the original size of this object)
